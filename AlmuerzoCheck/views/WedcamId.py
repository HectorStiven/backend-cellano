...
class BuscarEstudianteManual(generics.GenericAPIView):
    permission_classes = [AllowAny]
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not identificacion and not correo:
            return Response({
                "success": False,
                "detail": "Debe enviar al menos identificación o correo."
            }, status=status.HTTP_400_BAD_REQUEST)

        # Buscar coincidencia por identificación o correo
        estudiantes = T001Estudiantes.objects.all()
        estudiante_match = None

        for estudiante in estudiantes:
            if (identificacion and estudiante.identificacion == identificacion) or \
               (correo and estudiante.correo == correo):
                estudiante_match = estudiante
                break

        if estudiante_match:
            return Response({
                "success": True,
                "detail": "Estudiante encontrado.",
                "data": {
                    "id": estudiante_match.id,
                    "identificacion": estudiante_match.identificacion,
                    "tipo_documento": estudiante_match.tipo_documento,
                    "primer_nombre": estudiante_match.primer_nombre,
                    "segundo_nombre": estudiante_match.segundo_nombre,
                    "primer_apellido": estudiante_match.primer_apellido,
                    "segundo_apellido": estudiante_match.segundo_apellido,
                    "genero": estudiante_match.genero,
                    "fecha_nacimiento": estudiante_match.fecha_nacimiento,
                    "direccion": estudiante_match.direccion,
                    "telefono": estudiante_match.telefono,
                    "correo": estudiante_match.correo,
                    "grado": estudiante_match.grado,
                    "grupo": estudiante_match.grupo,
                    "jornada": estudiante_match.jornada,
                    "año_ingreso": estudiante_match.año_ingreso,
                    "estado": estudiante_match.estado,
                    "creditos": str(estudiante_match.creditos),
                    "creado_en": estudiante_match.creado_en,
                    "fotoId": request.build_absolute_uri(estudiante_match.fotoId.url) if estudiante_match.fotoId else None
                }
            }, status=status.HTTP_200_OK)
        else:
            return Response({
                "success": False,
                "detail": "No se encontró coincidencia en identificación ni correo."
            }, status=status.HTTP_404_NOT_FOUND)